...
export async function deletePost(apiKey, postId) {
  try {
    const res = await fetch(`${BLOTATO_BASE}/posts/${postId}`, {
      method: 'DELETE',
...
export async function publishPost(apiKey, payload) {
  try {
    ...
    // Try every known Blotato response shape for the post ID
    const postId = body?.post?.id ?? body?.id ?? body?.data?.id ?? body?.postId
      ?? body?.post?.postId ?? body?.result?.id ?? null;
    console.log('Blotato publish response body:', JSON.stringify(body));
    return { ok: true, postId, _raw: body };
  } catch (err) {
    return { ok: false, error: err.message };
  }
}
