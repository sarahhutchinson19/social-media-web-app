...
export async function publishPost(apiKey, payload) {
  try {
    ...
    // Try all known Blotato response shapes for the post ID
    const postId = body?.post?.id || body?.id || body?.data?.id || body?.postId || null;
    console.log('Blotato publish response:', JSON.stringify(body).slice(0, 200));
